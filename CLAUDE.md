...
| `scripts/submit-sitemap.py` | Resubmit the sitemap to Google Search Console (deploy step 6) |
| **Build Output** | |
...
The site is fronted by Cloudflare (zone `ad15899b816fb724b67ab95c75c3891e`), matching the other content zones: cache-everything rule (4h edge TTL), Smart Tiered Cache, and a User-Agent transform toward origin (Hostinger's shared-server WAF 403/429s crawler UAs — the transform bypasses it so GPTBot et al. reach the origin). **Origin 403s (open question, 2026-09-23).** Hostinger's origin refuses a large share of Cloudflare's page fetches by client IP: 44% of origin GETs for site pages in the week to 2026-09-22 (5,187 403s, including 6 of 142 Bingbot and 6 of 57 GPTBot fetches; verified Googlebot 46 of 46 fine). The hPanel CDN was switched off on 2026-09-23 to see if it was the cause; that is unconfirmed, and `GPTBot/1.0` still gets 429 on `/` at origin with it off. Compare the origin 403 share in Cloudflare analytics (`httpRequestsAdaptiveGroups`, `originResponseStatus`, GET on page paths) against that baseline. Deploy step 5 warms the cache so crawlers rarely reach the origin either way.
...
# Deployment
npm run deploy           # Build + rsync + IndexNow ping + Cloudflare purge + cache warm + Google sitemap resubmit
